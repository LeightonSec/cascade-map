...
# Python or one OS) silently drops conditional deps, which then fail under
# pip's implicit --require-hashes mode on the missing target. --universal
# captures every python-version and platform marker so the lockfile installs
# everywhere inside requires-python (>=3.10). --python-version 3.10 pins the
# compile to the CI matrix floor.
pin:
